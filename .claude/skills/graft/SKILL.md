...
### 1 · `./scripts/hardening/graft ask "<question>" --source`: locate + understand (the default)
...
### 2 · `./scripts/hardening/graft grep "<pattern>"`: exhaustive find
...
- Search a **short symbol name or literal**, not a full guessed signature: an
  ...
  and signature, keep the bare name) and retry `./scripts/hardening/graft grep` — do NOT switch to
...
### 3 · `./scripts/hardening/graft skeleton <file>`: a file's API at a glance
...
### 4 · `./scripts/hardening/graft callers <symbol>`: the exact edges
...
### 5 · `./scripts/hardening/graft map`: orientation for an unfamiliar repo or area
...
### 6 · Lifecycle: `./scripts/hardening/graft build` / `./scripts/hardening/graft check`
...
|---|---|---|
| Onboarding / "explain this codebase" | `./scripts/hardening/graft map`, then read the named hub cards | 1 |
| Understanding a flow ("how does X work") | `./scripts/hardening/graft ask "<flow>" --source` | 1 |
| Finding where a change belongs | `./scripts/hardening/graft ask "where is <behavior>" --source` | 1 |
| Editing a symbol you can already name | `./scripts/hardening/graft grep "<symbol>"`, edit at the `file:line` (skip `ask` — you know where it is) | 1 |
| Renaming / deleting / changing a signature | `./scripts/hardening/graft callers <sym> --depth 2` first | 1 |
| Refactor / multi-file change (before editing) | `./scripts/hardening/graft callers <sym> --depth all` — map every connected file, don't stop at the first | 1 |
| "What does this depend on?" | `./scripts/hardening/graft callers <sym> --direction out` | 1 |
| Finding every occurrence of a pattern | `./scripts/hardening/graft grep "<literal>"` | 1 |
| "What's the API of this file?" | `./scripts/hardening/graft skeleton <file>` | 1 |
| Debugging a failure in area X | `./scripts/hardening/graft ask "<symptom>" --source`, then `callers` on the suspect | 1–2 |
| Judging a diff's risk before merge | `./scripts/hardening/graft callers <changed sym> --depth 2` | 1 / symbol |
...
you already know where you're working, narrow with `./scripts/hardening/graft ask "<task>" --in <scope>/`.
...
- When the task already names the file or symbol to change, go straight there:
  `./scripts/hardening/graft grep "<symbol>"` for the exact `file:line`, then edit. Reserve
  `./scripts/hardening/graft ask` for when you don't yet know where the code lives — an `ask`
...
- If graft names a path that isn't on disk, its index is ahead of your checkout
  (a branch switch or unpulled move). Don't read the missing file — `./scripts/hardening/graft grep`
  the symbol to find where it lives now, or run `./scripts/hardening/graft build` to refresh.
